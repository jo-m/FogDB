/**
 * Canonical viridis colormap (256 samples) from matplotlib, stored as 0xRRGGBB
 * integers. Generated from matplotlib's `_viridis_data` list.
 */
...
/**
 * Interpolate linearly between two channel values.
 *
 * @param a Start channel value.
 * @param b End channel value.
...
/**
 * Map a normalized value to a viridis color.
 *
 * @param t Normalized value in [0, 1]; values outside are clamped.
...
export function viridis(t: number): string {
  ...
  const r = lerp((c0 >> 16) & 0xff, (c1 >> 16) & 0xff, frac)
  const g = lerp((c0 >> 8) & 0xff, (c1 >> 8) & 0xff, frac)
  const b = lerp(c0 & 0xff, c1 & 0xff, frac)
  return `rgb(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)})`
}

/**
 * A function mapping a raw value to a viridis color over a fixed domain.
 * A zero-width domain collapses to the midpoint color.
 */
export type ColorScale = (value: number) => string

/**
 * Build a viridis color scale for the given value domain.
 *
 * @param min Domain minimum.
 * @param max Domain maximum.
 * @returns A function mapping values in `[min, max]` to colors.
 */
export function makeViridisScale(min: number, max: number): ColorScale {
  const range = max - min
  if (range === 0) {
    return () => viridis(0.5)
  }
  return (value: number) => viridis((value - min) / range)
}

/**
 * Sample the viridis colormap into a CSS `linear-gradient` stop list, used to
 * render the color legend.
 *
 * @param steps Number of evenly spaced samples.
 * @returns A CSS gradient value such as `linear-gradient(90deg, #.., #..)`.
 */
export function viridisGradient(steps: number = 32): string {
  const stops: string[] = []
  for (let i = 0; i <= steps; i++) {
    stops.push(viridis(i / steps))
  }
